...
class BlindValidationMultiRoundTests(unittest.TestCase):
    ...
    def test_rounds_02_to_05_create_exactly_38_blind_cases(self) -> None:
        self.assertEqual([len(dataset.vectors) for dataset in self.datasets], [20, 7, 6, 5])
        self.assertEqual(len(self.packet.cases), 38)
        self.assertEqual(len({case.vector_id for case in self.packet.cases}), 38)
    ...
    def test_clean_38_case_predictions_compare_as_all_agree_but_never_promote(self) -> None:
        decisions = tuple(
        ...
        self.assertEqual(report.agree, 38)
    ...
    def test_one_missing_prediction_becomes_ambiguous_not_silently_dropped(self) -> None:
        ...
        self.assertEqual(report.total, 38)
        self.assertEqual(report.agree, 37)
