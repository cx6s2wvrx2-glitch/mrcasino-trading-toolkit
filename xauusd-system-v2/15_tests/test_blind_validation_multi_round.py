...
class BlindValidationMultiRoundTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.datasets = tuple(
            ...
            for round_no in (2, 3, 4, 5, 6, 7, 8, 9, 10)
        )
    ...
    def test_rounds_02_to_10_create_exactly_90_blind_cases(self) -> None:
        self.assertEqual([len(dataset.vectors) for dataset in self.datasets], [20, 7, 6, 5, 8, 10, 10, 4, 20])
        self.assertEqual(len(self.packet.cases), 90)
        self.assertEqual(len({case.vector_id for case in self.packet.cases}), 90)
    ...
    def test_clean_90_case_predictions_compare_as_all_agree_but_never_promote(self) -> None:
        decisions = tuple(
        ...
        self.assertEqual(report.agree, 90)
    ...
    def test_one_missing_prediction_becomes_ambiguous_not_silently_dropped(self) -> None:
        ...
        self.assertEqual(report.total, 90)
        self.assertEqual(report.agree, 89)
